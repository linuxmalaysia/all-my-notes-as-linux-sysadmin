...
3. **Pemprosesan Teks Aluran, Penyunting CLI & Amalan Keselamatan (Text Filters, CLI Editors & Security)**:
    ...
    - **Penyunting Teks CLI Pentadbiran**:
      ...
      - **GNU Nano / Pico**: Pintasan asas `Ctrl+O` (simpan), `Ctrl+X` (keluar), `Ctrl+W` (cari), `Ctrl+K`/`Ctrl+U` (potong/tampal). Pico menyokong sisipan fail `Ctrl+R`. Nano menambah keupayaan carian ganti `Ctrl+\`, penyerlahan sintaks, dan fail `.nanorc`.
    ...
    - **Pengurusan Pemboleh Ubah Persekitaran Shell**: Konfigurasi `$EDITOR`, `$VISUAL`, `$SYSTEMD_EDITOR`, `$SUDO_EDITOR` dalam `~/.bashrc`, `/etc/environment`, `/etc/profile` (dibaca oleh Bash untuk *login shells* sahaja), dan `/etc/profile.d/editor.sh`.
