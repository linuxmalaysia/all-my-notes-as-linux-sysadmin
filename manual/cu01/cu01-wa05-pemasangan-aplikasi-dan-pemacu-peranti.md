...
1. **Pengesahan Tandatangan Pakej & Operasi Asas Arahan `rpm`:**
   ...
   - `-q` (Query): Menyoal status pakej (`rpm -qa` untuk semua pakej, `rpm -qi <pakej>` untuk maklumat, `rpm -ql <pakej>` melepaskan senarai fail).
   - `-V` (Verify): Mengesan perbezaan fail terpasang berbanding pangkalan data RPM tempatan.
   ...
   ```bash
   # 1. Import kunci awam GPG rasmi dan sahkan tandatangan digital pakej RPM
   sudo rpm --import /etc/pki/rpm-gpg/RPM-GPG-KEY-official
   rpmkeys --checksig nmap-7.95-1.x86_64.rpm || { echo "Pengesahan GPG gagal atau NOKEY!"; exit 1; }

   # 2. Pemasangan disyorkan (DNF menyelesaikan kebergantungan automatik)
   sudo dnf install -y ./nmap-7.95-1.x86_64.rpm

   # 3. Pemasangan aras rendah (RPM)
   sudo rpm -Uvh nmap-7.95-1.x86_64.rpm

   # 4. Semak maklumat dan fail yang dimiliki oleh sesuatu pakej
   rpm -qi nmap
   rpm -ql nmap

   # 5. Cari pakej RPM yang memiliki fail spesifik di dalam sistem
   rpm -qf /usr/bin/nmap

   # 6. Pengesahan integriti fail terpasang berbanding pangkalan data RPM tempatan
   rpm -V nmap
   rpm -Va
   ```
...
3. **Pengompilan Manual daripada Arkib Kod Sumber Tarball (`.tar.gz` / `.tar.zst`):**
   ...
   - **Langkah Keselamatan:** Dapatkan fail checksum (`sha256sum`) dan kunci GPG daripada saluran rasmi yang disahkan. Sahkan tandatangan manifesto/kunci sebelum menjalankan `sha256sum -c`.
   ...
   ```bash
   # 1. Sahkan kunci GPG & integriti checksum sha256sum daripada saluran rasmi
   gpg --verify sampel-aplikasi-1.0.tar.gz.sha256.asc sampel-aplikasi-1.0.tar.gz.sha256
