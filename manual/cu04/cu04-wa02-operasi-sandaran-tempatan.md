...
#### B. Pelaksanaan Amali Pemampatan `zstd` & Penjanaan Manifest SHA-256 (Standard 2026)
`zstd` (Zstandard) merupakan algoritma pemampatan masa nyata berasaskan bebenang berbilang (*multi-threaded*) berprestasi tinggi.

```bash
# Membina arkib terpatat zstd daripada direktori /etc dan /var/log menggunakan nama stabil system_config_20260817.tar.zst
sudo tar -I 'zstd -T0 -19' -cvf /mnt/backup/system_config_20260817.tar.zst /etc /var/log

# Menjana manifest integriti SHA-256 sepadan
...
sudo tar -I zstd -xvf /mnt/backup/system_config_20260817.tar.zst -C /tmp/restored_logs/
```

#### C. Pemampat Tradisional & Moden lain
```bash
# Pemampatan Gzip (.tar.gz)
tar -czvf /mnt/backup/system_config_20260817.tar.gz /etc

# Pemampatan XZ (.tar.xz) - kadar mampatan tinggi
tar -cJvf /mnt/backup/system_config_20260817.tar.xz /etc
```
...
> ⚠️ **Amaran Cermin Memusnahkan (Destructive Mirror Warning):** Opsi `--delete-after` akan memadam fail di destinasi jika ia tiada di sumber. Sebelum menggunakan `--delete-after`, pastikan terdapat polisi simpanan (retention policy) atau sandaran berasaskan cermin snapshots (seperti ZFS/Btrfs snapshot) bagi memastikan data terpelihara.
...
# --delete-after: Memadam fail di destinasi jika dipadam di sumber (guna dengan cermin/snapshot)
...
Mewujudkan servis sandaran di `/etc/systemd/system/local-backup.service` dengan syarat pelekapan storan:
```ini
[Unit]
...
- [ ] Berjaya menyegerakkan direktori menggunakan `rsync` dengan perlindungan retention/snapshot.
- [ ] Memahami dan mengaplikasikan prinsip sandaran 3-2-1.
- [ ] Mampu mengkonfigurasi automasi sandaran berkala menerusi `cron` atau `systemd.timer` dengan `RequiresMountsFor`.
