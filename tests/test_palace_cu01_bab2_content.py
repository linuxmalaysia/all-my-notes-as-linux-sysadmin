"""Tests for the Bab 2 (CU01) content added/updated in this change set.

Covers:
  - The four new Sovereign Markdown Palace knowledge nodes under palace/cu01/
...
These tests follow the same lightweight string-based frontmatter parsing
convention used by tests/test_okf_compliance.py (no PyYAML dependency).
"""
...
def split_frontmatter(content):
    ...
    Mirrors the parsing approach used in test_okf_compliance.py.
    """
...
# ---------------------------------------------------------------------------
# New palace/cu01 knowledge nodes
# ---------------------------------------------------------------------------

@pytest.mark.parametrize("relpath", NEW_PALACE_NODES)
def test_new_palace_node_exists(relpath):
    assert (REPO_ROOT / relpath).is_file(), f"Expected new knowledge node {relpath} to exist."


@pytest.mark.parametrize("relpath", NEW_PALACE_NODES)
def test_new_palace_node_has_okf_frontmatter(relpath):
    content = read(relpath)
    frontmatter, _ = split_frontmatter(content)

    assert "okf_version:" in frontmatter, f"{relpath} missing okf_version key."
    assert "type: knowledge_node" in frontmatter, f"{relpath} should be type: knowledge_node."
...
@pytest.mark.parametrize("relpath", NEW_PALACE_NODES)
def test_new_palace_node_timestamp_is_valid_iso8601(relpath):
...
@pytest.mark.parametrize("relpath", NEW_PALACE_NODES)
def test_new_palace_node_resource_path_matches_file_location(relpath):
    frontmatter, _ = split_frontmatter(read(relpath))
...
@pytest.mark.parametrize("relpath", NEW_PALACE_NODES)
def test_new_palace_node_has_sovereign_footer(relpath):
...
def test_hardware_bios_uefi_node_mentions_canonical_2026_distros():
    content = read("palace/cu01/keperluan-perkakasan-dan-bios-uefi.md")
...
def test_installation_procedure_node_covers_lvm_and_target_distros():
    content = read("palace/cu01/prosedur-pemasangan-ubuntu-almalinux.md")
...
def test_luks2_encryption_node_covers_key_compliance_topics():
    content = read("palace/cu01/penyulitan-cakera-luks2-pejabat.md")
...
def test_post_install_node_covers_package_management_and_hardening():
    content = read("palace/cu01/pasca-pemasangan-dan-driver.md")
...
def test_openwiki_topic01_has_eight_numbered_sections():
    ...
    # Ensure there isn't a stray 9th section left over from a bad merge.
...
def test_openwiki_topic01_no_longer_references_stale_2024_distro_versions():
    content = read(OPENWIKI_TOPIC_01)
    # These 2024-era version references should have been fully replaced.
