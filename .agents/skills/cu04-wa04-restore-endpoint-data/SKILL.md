...
## 🎯 Skill Overview
This AI agent skill guides the execution of data integrity verification using `sha256sum` (for detection of accidental data corruption during transmission or storage) or detached GPG signatures (for tamper-protection verification), selective file extraction from `system_config_20260817.tar.zst` archives, permission preservation, and bare-metal disaster recovery procedures.

---

## 🛠️ Execution Procedure

### 1. Integrity & Corruption Detection (`sha256sum` & `gpg`)
> ℹ️ Note: A co-located `sha256sum` manifest verifies against accidental file corruption. For tamper-proof verification against malicious modification, verify a detached GPG signature (`gpg --verify system_config_20260817.tar.zst.sig`).

```bash
# Verify checksum manifest for corruption detection
cd /mnt/backup/
sha256sum -c system_config_20260817.tar.zst.sha256

# Verify detached GPG signature for tamper protection (if available)
...
3. Extract bare-metal archive: `sudo tar -I zstd -xvf /mnt/backup/system_config_20260817.tar.zst -C /mnt`.
